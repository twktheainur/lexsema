...
public class KroneckerTensorProductOperator implements BinaryTensorOperator{
    ...
    private static int[] computeProductResultShape(int[] aShape, int[] bShape){
        ...
        int[] resultShape = new int[aLength + 1];
        int i;
        for(i=0; i< bLength;i++){
            resultShape[i] = bShape[i];
        }
        int aElems = 1;
        for (int shapeAt : aShape) {
            aElems *= shapeAt;
        }
        resultShape[i] = aElems;
        return resultShape;
    }
}
