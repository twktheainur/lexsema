    ...
    public double computeScore(Configuration c, double[] window) {
        ...
        for(int i=0;i<c.size();i++){
            for(int j=0;i<c.size();j++){
                if(window[j]>0.0001d){
                    List<String> defA = document.getSense().get(i).get(c.getAssignment(i)).getSignature();
                    List<String> defB = document.getSense().get(j).get(c.getAssignment(j)).getSignature();
