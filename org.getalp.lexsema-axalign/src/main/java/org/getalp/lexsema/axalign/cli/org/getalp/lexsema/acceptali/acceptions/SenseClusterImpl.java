...
class SenseClusterImpl implements SenseCluster {
    ...
    public String toString() {
        ...
        for(int i=0;i<members.size();i++) {
            retBuilder.append(String.format("\t%s (%s) -- %s %s", URIUtils.getCanonicalURI(members.get(i).getId()), weights.get(i),members.get(i).getSemanticSignature(),System.lineSeparator()));
        }
