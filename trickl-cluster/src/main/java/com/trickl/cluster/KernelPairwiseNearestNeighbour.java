...
public class KernelPairwiseNearestNeighbour implements ClusterAlgorithm {
    ...
    public void cluster(final DoubleMatrix2D kernel, int clusters) {
        ...
        for (int itr = 0; itr < n - clusters; itr++) {
            ...
            gamma.assign(prototypeVectors.viewColumn(bestMergePair.newCluster), DoubleFunctions.plusMultSecond(n_a / (n_a + n_b)));
            gamma.assign(prototypeVectors.viewColumn(bestMergePair.oldCluster), DoubleFunctions.plusMultSecond(n_b / (n_a + n_b)));
