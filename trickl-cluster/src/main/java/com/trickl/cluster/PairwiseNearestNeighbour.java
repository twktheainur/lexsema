...
public class PairwiseNearestNeighbour implements ClusterAlgorithm {
    ...
    private DoubleStatistic.VectorVectorFunction distanceMeasure = DoubleStatistic.EUCLID;
    ...
    public void cluster(DoubleMatrix2D data, int clusters) {
        ...
        for (int itr = 0; itr < n - clusters; itr++) {
            ...
            weightedMeanCodeVector.assign(codeVectors.viewRow(bestMergePair.newCluster), DoubleFunctions.plusMultSecond(n_a / (n_a + n_b)));
            weightedMeanCodeVector.assign(codeVectors.viewRow(bestMergePair.oldCluster), DoubleFunctions.plusMultSecond(n_b / (n_a + n_b)));
